...
We know that the above objectives are not mathematically equivalent to the original one, so plugging such a thing on top of $$\phi$$ is not exactly doing SVM. Here we try to refine the soft-margin objective presented above to get it as close to the original as possible. Observe that the first constraint simply establish a domain restriction on $$\xi$$, but we know that auto differentiation will produces gradients that push $$\xi$$ below $$0$$ whenever needed. So an obvious thing to do is to clip the gradient whenever it falls below zero.
